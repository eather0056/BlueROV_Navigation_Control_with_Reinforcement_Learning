...
if args.randomization == True:
    policy_net, value_net, running_state = pickle.load(
        open(os.path.join(assets_dir(), 'learned_models/{}_trpo_rand_Env_C.p'.format(args.env_name,
                                                                      args.hist_length)), "rb"))
else:
    policy_net, value_net, running_state = pickle.load(
        open(os.path.join(assets_dir(), 'learned_models/{}_trpo_norand_10_250iters.p'.format(args.env_name,
