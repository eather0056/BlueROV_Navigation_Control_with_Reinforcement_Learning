...
def main(args=None):
    ...
    # Intial agent
    agent = SAC(env[0].num_inputs, env[0].action_space, args)

    

    # Initial ensemble model
    state_size = np.prod(env[0].num_inputs)
    action_size = np.prod(env[0].action_space.shape[0])
    print(env[0].action_space.shape[0])
